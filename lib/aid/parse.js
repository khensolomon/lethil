...
export function cli(q) {
  ...
  const params = [];
  if (sfe.length == 2) {
    // NOTE: split by space or & but not within quotes
    // const queries = sfe[1].match(/(?:[^\s"']+|['"][^'"]*["'])+/g);
    const queries = sfe[1].match(/(?:[^\s|&"']+|['"][^'"]*["'])+/g);
    // let hasEqualTo = false;
    // let name = "";
    // const queries = sfe[1].split(" ");
    // for (const arg of queries) {
    //   hasEqualTo = arg.includes("=");
    //   if (hasEqualTo) {
    //     params.push(arg);
    //   } else {
    //     if (name == "") {
    ...
    if (queries) {
      for (let index = 0; index < queries.length; index++) {
        ...
        // console.log("arg?", arg);
        // let hasEqualTo = arg.includes("=");

        if (arg.includes("=")) {
          // NOTE: split only by the first =
          const [a, b] = arg.split(/=(.*)/s);
          // params.push(arg);
          params.push(a + "=" + b.replace(/^["'](.+(?=["']$))["']$/, "$1"));
          // params.push(a + "=" + b);
        } else {
          if (name == "") {
            name = arg;
