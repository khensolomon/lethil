/**
 * name TypeOfParseURL
 *
 * @extends {URL}
 */
export class ParseURL extends URL {
  /**
   * by providing custom base param, expected a valid url
   * @param {string} url
   * @param {string} [base="path://local"] - is optional, default "path://local"
   * @default {path://local} base
   */
  constructor(url, base = "path://local") {
    super(url, base);
  }

  get query() {
    // return [...new URLSearchParams(this.search).entries()].reduce((prev, [key,val]) => {prev[key] = val; return prev}, {});
    return Object.fromEntries(this.searchParams);
  }
}

/**
 * @typedef {ParseURL} TypeOfParseURL
 * parse URL and return params as object `query(string)`.
 * @param {string} str
 * @returns {ParseURL}
 */
export function url(str) {
  return new ParseURL(str);
}

/**
 * parse command to url.
 * @param {string[]|string} q
 * @returns string
 * @example
 * param/id?a=1 b=2 -> param/id?a=1&b=2
 * param/id?a=1--b=2 -> param/id?a=1&b=2
 * param/id a=1 b=2 -> param/id?a=1&b=2
 * param/id a:1 b:2 -> param/id?a=1&b=2
 */
export function cli(q) {
  let cml = "";
  if (q instanceof Array) {
    cml = q.join(" ");
  } else {
    cml = q;
  }

  if (cml && /^\//.test(cml) == false) {
    cml = "/" + cml;
  }

  if (q && cml.includes("?") == false) {
    cml = cml.replace(/--/, "?");
  }
  // NOTE: replace every -- but except first occurrence
  // cml = cml.replace(/(?!^)--/g, " ");

  // /(?:[^\s|&"']+|['"][^'"]*["'])+/g;
  const formated = cml.replace(/--/g, "&");
  // .replace(/(?!^)--/g, "&")
  // .replace(/--/, "&");
  // .replace(/\/\?/, "?")
  // .replace(/\/\&/, "&")
  // .replace(/\?\&/, "?")
  // .replace(/\&\&/g, "&")
  // .replace(/ \&/g, "&");
  // .replace(/(:)(?=(?:[^"']|['"][^'"]*["'])*$)/g, "=");
  // .replace(/(:)(?=(?:[^"']+|['"][^'"]*["'])*$)/g, "=");
  // .replace(/:/g, "=");

  // "".replace(/(:)(?=(?:[^"]|"[^"]*")*$)/g, "=");
  // "".replace(/(:)(?=(?:[^"']+|['"][^'"]*["'])*$)/g, "=");

  const sfe = formated.split("?");

  const uri = sfe[0].trim().replace(/ /g, "/");
  // const uri = sfe[0].trim();

  const params = [];
  // const params = sfe[1].replace(/%%/g, "&");
  if (sfe.length == 2) {
    // NOTE: split by [\s] or [&] but not within quotes
    // const queries = sfe[1].match(/(?:[^\s"']+|['"][^'"]*["'])+/g);
    const queries = sfe[1].match(/(?:[^\s|&"']+|['"][^'"]*["'])+/g);

    let name = "";
    if (queries) {
      for (let index = 0; index < queries.length; index++) {
        // NOTE: remove first and last quotes [single or double]
        const arg = queries[index];
        // const arg = queries[index].replace(/%%/g, "&");
        if (arg.includes("=")) {
          // NOTE: split only by the first =
          const [k, v] = arg.split(/=(.*)/s);
          params.push(k + "=" + v.replace(/^["'](.+(?=["']$))["']$/, "$1"));
          name = k;
        } else {
          if (name == "") {
            name = arg;
          } else {
            const v = arg.replace(/^["'](.+(?=["']$))["']$/, "$1");
            if (params.length) {
              const last = params.length - 1;
              params[last] += " " + v;
            } else {
              params.push(name + "=" + v);
            }
          }
        }
      }
    }
  }

  if (params.length) {
    return uri + "?" + params.join("&");
  }

  return uri;
}

/**
 * @param {string|string[]} strOrArray
 * @returns RegExp
 * @example
 * hostNameRegex("http://localhost:8081") -> /^http\:\/\/localhost\:8081$/i
 * hostNameRegex("localhost.local") -> /^localhost\.local$/i
 * hostNameRegex("http://*.localhost.local") -> /^http\:\/\/([^.]+)\.localhost\.local$/i
 * hostNameRegex(["a","b","c"]) -> /a|b|c/i
 */
export function hostNameRegex(strOrArray) {
  var ASTERISK_REGEXP = /\*/g;
  var ASTERISK_REPLACE = "([^.]+)";
  var END_ANCHORED_REGEXP = /(?:^|[^\\])(?:\\\\)*\$$/;
  var ESCAPE_REGEXP = /([.+?^=!:${}()|[\]/\\])/g;
  var ESCAPE_REPLACE = "\\$1";

  var src = "";

  if (strOrArray.constructor === Array) {
    src = strOrArray
      .map(function (e) {
        return String(e)
          .replace(ESCAPE_REGEXP, ESCAPE_REPLACE)
          .replace(ASTERISK_REGEXP, ASTERISK_REPLACE);
      })
      .join("|");
  } else {
    src = String(strOrArray)
      .replace(ESCAPE_REGEXP, ESCAPE_REPLACE)
      .replace(ASTERISK_REGEXP, ASTERISK_REPLACE);
    if (src[0] !== "^") src = "^" + src;
    if (!END_ANCHORED_REGEXP.test(src)) src += "$";
  }
  return new RegExp(src, "i");
}

/**
 * @param {string} e
 * @returns any
 * @example
 * .hostNameExec(*.example.com) -> example.com
 * .hostNameExec(http://www.*.example.com -> example.com
 */
export function hostNameExec(e) {
  // return /(?:[\w-]+\.)+[\w-]+/.exec(e);
  return /(?:[\w-]+\.)+[\w-]+/.exec(e)?.[0];
}

class environmentParser {
  /**
   * @private
   */
  val = "";

  /**
   * @param {*} val
   */
  constructor(val) {
    this.val = val;
  }

  /**
   * @example restrict("d1v:l1;i1o:i1;a1n:a1;w1b:w1") -> {d1v:"l1",i1o:"i1",a1n:"a1",w1b:"w1"}
   */
  restrict() {
    return context(this.val);
  }

  /**
   * @example listen("host:localhost;port:8082") -> {host:"localhost",port:8082}
   */
  listen() {
    return context(this.val);
  }

  /**
   * Remove duplicate
   * @returns {RegExp}
   * @example referer("locahost.local;example.com;example.com") -> /localhost\.local|example\.com/i]
   */
  referer() {
    // return aid.fire.array.unique($.user.referer.split(';')).map(aid.parse.hostNameRegex);
    // return this.val
    //   .split(";")
    //   .filter((value, index, self) => self.indexOf(value) === index)
    //   .map(hostNameRegex);
    var uniqueArray = this.val
      .split(";")
      .filter((value, index, self) => self.indexOf(value) === index);
    return hostNameRegex(uniqueArray);
  }
}

/**
 *  {{[k: any]: any}}
 * @param {String|Buffer} str
 * @returns {any}
 */
export function environment(str) {
  /**
   * @type {any} result
   */
  var result = {};
  // const parser = new environmentParser();
  if (Buffer.isBuffer(str)) str = str.toString();
  var e = str
    .replace(/\r\n/g, "\n")
    .split("\n")
    .filter((e) => /^[#;]/.test(e) == false && /=/.test(e));
  for (var i = 0; i < e.length; i++) {
    if (e[i]) {
      /** @type {any} j */
      var j = e[i].split(/=(.*)/g).map((e) => e.trim());
      if (j.length > 1) {
        const parser = new environmentParser(j[1]);
        /** @type {keyof parser} name */
        const name = j[0];
        if (name in parser && typeof parser[name] == "function") {
          result[j[0]] = parser[name]();
        } else {
          result[j[0]] = j[1];
        }
      }
    }
  }
  return result;
}

/**
 * @param {String} e
 * @example context('a:1;b:2;c:3') -> {a:1,b:2,c:3}
 * @returns Object
 */
export function context(e) {
  return e
    .split(";")
    .map((e) => e.split(":"))
    .filter((e) => e.length > 1)
    .reduce((o, i) => Object.assign(o, { [i[0]]: i[1] }), {});
}

/**
 * @param {string} value
 * @example explode('one two') -> [one,two]
 * @returns {string[]}
 */
export function explode(value) {
  return value.trim().split(/\s+/);
}

/**
 * @param {any} value
 * @example count('one two') -> 2
 * @returns number
 */
export function count(value) {
  return explode(value).length;
}

/**
 * cookie Parser -> this.headers.cookie
 * @param {any} str
 * @example cookier('foo=bar; bar=foo') -> {foo:bar, bar:foo}
 * @returns {any}
 */
export function cookie(str) {
  if (typeof str == "string") {
    return str
      .split(";")
      .filter((e) => e.trim())
      .map((e) => e.trim().split("="))
      .reduce(
        (accum, current) =>
          Object.assign(accum, {
            [current[0]]: decodeURIComponent(current[1]),
          }),
        {}
      );
  }
  return {};
}

/**
 * cookie Stringify -> this.headers.cookie
 * @param {any} o - of an Object
 * @example cookieStringify({a:1, b:4}) -> "a=1; b=4"
 */
export function cookieStringify(o) {
  return Object.entries(o)
    .map(([k, v]) => k + "=" + encodeURIComponent(v))
    .join("; ");
}

/**
 * parse.csv
 * @param {string | Buffer} str - of an Object
 * @example csv("a\r\nb") -> ["a","b"]
 */
export function csv(str) {
  return str
    .toString()
    .split(/\r\n/)
    .filter((e) => e.trim());
}
