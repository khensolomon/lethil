...
function getPrototype(urlOptions) {
  if (
    ...
    urlOptions.charAt(4).localeCompare("s") >= 0
  ) {
    return https;
  } else if (
    typeof urlOptions == "object" &&
    urlOptions.port &&
    urlOptions.port == 443
  ) {
    // port: 443,
    ...
    const req = ask(urlOptions, (res) => {
      ...
      res.on("end", () => {
        ...
        try {
          ...
          if (
            urlOptions.headers &&
            urlOptions.headers["Content-Type"].indexOf("application/json") >= 0
          ) {
...
export class gistData {
  ...
  async get(fileName = "", options) {
    try {
      const res = await this.request(options).catch((e) => {
        throw e;
      });
