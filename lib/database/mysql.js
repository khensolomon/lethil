...
export default class database {
  ...
  get configuration() {
     ...
     */
    const res = {};
    var head = this.config.split("//");
    if (head.length == 2) {
      var by = head[1].split("@");
      if (by.length == 2) {
        var [user, password] = by[0].split(":");
        if (user) {
          res["user"] = user;
        }
        if (password) {
        ...
        var [host, name] = by[1].split("/");
        if (host) {
          res["host"] = host;
        }
        if (name) {
          if (name.includes("?")) {
            const [dbHost, options] = name.split("?");
            res["database"] = dbHost;
            if (options) {
              for (var param of options.split("&")) {
  ...
  connect() {
    return new Promise(async (resolve, reject) => {
      // NOTE: wait to connect and initializing
      await new Promise((res) => setTimeout(res));
      if (
        !this.engine ||
        (typeof this.engine == "object" &&
          !this.engine.hasOwnProperty("createPool"))
      ) {
        return reject("no MySQL provided");
      }
      if (!this.config) {
      ...
      if (!this.pool) {
        this.pool = this.engine.createPool(this.configuration);
      }
      /**
       * @type {{():Promise<any>}}
       */
      ...
      connector()
        ...
        .catch((error) => {
          ...
          if (
            ["ECONNRESET", "ECONNREFUSED", "PROTOCOL_CONNECTION_LOST"].indexOf(
              error.code
            ) < 0
          ) {
