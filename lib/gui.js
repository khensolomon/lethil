...
app.on("request", async (req, res) => {
  ...
  try {
    ...
    if (route) {
      ...
      // res.locals.appDescription = config.app.description;
      ...
      //   res.locals.referer = req.headers.host == ref.host;// || config.app.referer.filter((e)=>e.exec(ref.host)).length > 0;
      //   res.locals.host = ref.protocol+'//'+req.headers.host;
      // }

      await handler.filter(route.middleware, req, res);

      for (let index = 0; index < config.route.name.length; index++) {
        const name = config.route.name[index];
        if (name && name != "_") {
          res.locals[name] = config.route.menu.filter(
            (e) => e.group == name && e.text
          );
        }
      }
