/**
 * @property {[k: string]: string}
 */
export const starter = {
  env: ".env",
  json: "package.json",
  // config: 'config.js',
  // command: 'command.js',
  // main: "index.js",
  // middleware: 'middleware.js',
  // route: 'route.js'
  // initiator: 'initiator.js',
};

/**
 * @property{[k: string]: string}
 */
export const environment = {
  mysqlConnection: null,
  mongoConnection: null,
};

/**
 * typedef {[typeof app] & any} configuration

 * return {Child}
 * type {[configuration]}
 * type {}
 * namespace
 * property {[k: string]: any}
 * property {[typeof app] & {[k: string]: any}}

 * property {any}
 * property {[configuration]}
 * property {boolean} app.development - if process.env.NODE_ENV empty or process.env.NODE_ENV is not production
 */
export const app = {
  name: "",
  version: "",
  description: "",
  /**
   * @readonly
   */
  get development() {
    return !process.env.NODE_ENV || process.env.NODE_ENV.trim() != "production";
  },
  merge() {},
  bucket: "",
  storage: "",
  media: "",
  lethil: {
    version: "",
  },
  /**
   * @description expected from .env
   */
  speechUrl: "",
  /**
   * type {RegExp[]} referer
   * @type {RegExp} referer
   */
  referer: new RegExp(""),
  /**
   * @type {any} restrict
   */
  restrict: "",
  /**
   * @type {any} listen
   */
  listen: {
    host: "127.0.0.1",
    port: 80,
  },
  dir: {
    root: "",
    static: "static",
    assets: "assets",
    views: "views",
    routes: "routes",
  },
  /**
   * @type {any[]} locale
   */
  locale: [
    // {id:'en',name:'English',default:true}
  ],
  /**
   * Used in deployment, get from package.json
   * @example
   * transferEnvironment
   * createOrUpdate
   */
  env: {
    app: {
      name: "",
      root: "",
      repo: "",
    },
  },
  /**
   * Used in deployment, get from .env
   * @example
   * transferEnvironment
   */
  APP_NAME: "",
  APP_ROOT: "",
  SSH_USER: "",
  SSH_HOST: "",
};

/**
 * @type {string[]}
 * @example
 * process.argv.splice(2)
 * npm start [?]
 * npm run [?]
 * node run [?]
 */
export const param = process.argv.splice(2);

/**
 * @property {[k: string]: any}
 */
export const middleware = {};

/**
 * @property {[k: string]: any}
 */
export const view = {
  /**
   * @type {any} engine
   */
  engine: null,
  /**
   * @type {string} engine
   */
  extension: ".pug",
};

/**
 * @property {string[]} name
 * @property {Array<any>} menu
 * @property {Array<any>} list
 */
export const route = {
  /**
   * @type {any} active
   */
  active: {},
  /**
   * @type {string[]} name
   */
  name: [],
  /**
   * @type {Array<any>} menu
   */
  menu: [],
  /**
   * @type {any[]} list
   */
  list: [],
  /**
   * @type {any[]} list
   */
  middleware: [],
};
