 ...
 * @property {string} method - [get, post, delete]
 * @property {string} url - request URL
 * @property {any} params - ?
 * @property {any} query - ?
 * @property {typeof aid.parse.ParseURL} route - ????
 *
 ...
 * @typedef {(req:TypeOfRouteRequest)=>void} TypeOfRouteRestCli - abc
 * type {TypeOfRouteRequest} req - see {@link TypeOfCommandRequest}
 *
 * property {ServerOptions<typeof Request, typeof ServerResponse>.IncomingMessage?: typeof Request | undefined}
 *
 * url
...
class Response extends ServerResponse {
  ...
  setHeaders(key, val = "") {
    if (typeof key == "string" && val) {
      ...
      // let abc = Object.entries(key);
      // for (let index = 0; index < abc.length; index++) {
      //   const element = abc[index];
      //   this.setHeader(element[0], element[0]);
      // }
    }
    return this;
  }
  ...
  render(filename, data = {}) {
    ...
    if ($.view.engine == null) {
      ...
      const abc = Object.assign({}, this.locals, data);
      var html = $.view.engine.renderFile(file, abc);
      this.end(html);
...
export function refresh(listener) {
  ...
  //   { IncomingMessage: Request, ServerResponse: http.ServerResponse },
  //   listener
  // ));
