...
function routeHandler(menu, fn, method, middleware = null) {
  ...
  if (route) {
    ...
    $.route.list.push({ url: menu.url, type: { [method]: routeMethod } });
    $.route.list.sort((a, b) => b.url.localeCompare(a.url));
  }
}
...
export class Router {
   ...
   * @param { any } rest
   */
  dispatch(method, url, rest) {
    routeTerminal(
      method,
      routeObject(this.routeId, this.routePrefix, url),
      rest
    );
  }

  // /**
  //  * @param { string | {url:string, text:string, group:string} } url - {@link TypeOfRouteRest}
  //  * @param {...TypeOfRouteRest } rest
  //  */
  // get(url, ...rest) {
