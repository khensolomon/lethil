#!/usr/bin/env python3
"""
Production server bootstrap — Docker Swarm, Cloudflare Tunnel, landing nginx,
firewall, and rclone. Idempotent.

version: 26.06.13-3

Modes
    Standard, no Cloudflare:   sudo python3 setup.py
    Manual tunnel token:       sudo python3 setup.py --cloudflare-token <TOKEN>
    Automated:                 pass the Cloudflare and R2 credentials plus the
                               domains as flags (see the template below, or run
                               --show-command). Creates the tunnel, routes,
                               DNS, and Access app/policy; zone IDs resolve from
                               the domain names.

    Add --force to overwrite an existing same-named tunnel, or --dry-run to
    print the actions without making changes.

Notes
    --app-domain is DOMAIN:TARGET and repeatable. DOMAIN is the public
    hostname; TARGET is the internal URL the tunnel forwards to (e.g.
    http://localhost:8000).

    --cf-api-token needs Zone:DNS:Edit (all managed zones),
    Account:Cloudflare Tunnel:Edit, and Account:Access Apps and Policies:Edit.

    --cf-service-token-id / --cf-service-token-secret reference an EXISTING
    Access service token (created once under Zero Trust -> Access -> Service
    Auth). This script attaches it to the ssh.<admin-domain> policy and never
    creates it. One token serves both this script and the deploy pipeline
    (each app's .env -> secrets.py). To rotate: replace the token in the
    dashboard, update each .env, run secrets.py --push, then re-run setup.py to
    re-attach it.

Show the install command (--show-command)
    Runs in the repo, not on the server. Reads ../.env, fills the template
    below, prints the command to stdout, and writes nothing:
        python3 server/setup.py --show-command

    Only the {PLACEHOLDERS} are filled, keyed by .env variable name -- the flag
    spelling stays literal, so {R2_ACCESS_SECRET} fills the
    --r2-secret-access-key value. Domain, tunnel name, and app domains are
    literals edited in the template. To extend it: add a `--flag "{ENV_KEY}"`
    line, add ENV_KEY to ../.env, and double any literal brace that is not a
    placeholder ("{{" / "}}").
...
sudo python3 setup.py \\
  ...
  --domain                   "lethil.me" \\
  --tunnel-name              "tunnel-self-hosted" \\
  --app-domain               "test.lethil.me:http://localhost:80" \\
  --app-domain               "zaideih.lethil.me:http://localhost:3010" \\
  --app-domain               "myordbok.lethil.me:http://localhost:3020" \\
  --add-ca-public-key
</command>
"""
...
# Captured at import so --show-command can slice the <command> template out of
# the docstring. (python3 -OO strips docstrings, leaving this None — handled in
# show_command().)
MODULE_DOC = __doc__
...
def setup_logging():
    """Configure logging for a real setup run.

    Not done at import time: the /var/log/setup.log handler needs write access
    (root), and read-only modes like --show-command must run unprivileged.
    main() calls this only on the real path. Logs to stdout always; adds the
    file handler when writable, and degrades to stdout-only otherwise instead
    of crashing.
    """
    ...
    try:
        ...
        print(f"Note: file logging to {LOG_FILE} disabled ({e.strerror}); "
...
def parse_args():
    ...
    parser.add_argument('--show-command', action='store_true',
                        help="Read ../.env and print the full setup command to "
                             "stdout (from the <command> template in the docstring). "
                             "Read-only: needs no sudo and changes nothing.")
...
def dns_upsert(api_token, zone_id, hostname, tunnel_id, force=False):
    ...
    hostname already exists (error 81053). The script fetches all record types at
    ...
    # the same name are fine and stay untouched.
    ...
    # Otherwise A/AAAA records (and possibly a stale CNAME) are blocking the CNAME.
...
def verify_service_token(api_token, account_id, service_token_id):
    ...
    account before wiring it into a policy. The API does not return the
    secret on a list call (the secret is only ever returned at creation
    time), but it does return the client_id to match against.
...
def access_app_upsert(api_token, account_id, hostname, app_name=None,
    ...
    matters because the dashboard allows multiple apps with the
    same name but different domains, and what actually matters is
    ...
    payload = {
        ...
        # access, which is not needed here but does no harm — the policy
...
def access_policy_ensure(api_token, account_id, app_id, service_token_uuid,
    ...
    for policy in existing.get('result', []) or []:
        if policy.get('decision') != 'non_identity':
            continue
        # Look for a service_token include rule referencing the token.
...
def setup_docker(force=False, dry_run=False):
    ...
    # Add the invoking user to the 'docker' group to allow running `docker`
    # without sudo. This is post-install hygiene — every Docker CLI command
    # in the rest of this script runs as root (under sudo), so this
    # step doesn't affect setup.py itself; it's purely for the operator's
    # interactive use afterwards.
    #
    # usermod -aG is idempotent (re-adding a user to an already-joined
    # group is a no-op), so this is safe on every re-run. Skipped if the
    ...
    # has to run `newgrp docker` (or log out and back in) to
    # activate the group in the current terminal session. A hint is printed
    ...
    if target_user == 'root':
        ...
        if dry_run:
            ...
            print(f"   To use docker without sudo in the current shell, run:")
...
def setup_landing(force=False, dry_run=False):
    ...
    The container:
      ...
      - Mounts nginx.conf read-write — vanilla nginx doesn't mutate it, but
        write access stays open for future updates of the image that
    ...
    # until the container tries to start, producing a cryptic error. Checked
    ...
    # Allow the landing container — it will be replaced by --force-recreate.
...
def print_post_install_summary(robotic_setup=False, tunnel_id=None,
    ...
    if robotic_setup:
        ...
        if access_protected_hostname:
            service_auth_note = (
                f"\n   The supplied service token is now attached to a "
...
{app_list}
   ...
   /opt/apps/html. Drop the SPA into that directory to replace the
   placeholder.
{service_auth_note}
...
""")
    ...
    print("""\
NEXT STEPS
  1. Drop the SPA build output into /opt/apps/html/ to replace the
...
# ─── Main ─────────────────────────────────────────────────────────────────────
def load_env(path):
    """Read a KEY=VALUE .env into a dict (stdlib only).

    Blank lines and #-comments are skipped; surrounding quotes are stripped.
    Trailing inline comments are not stripped (no value uses them). If
    secrets.py grows a richer parser, import that instead to avoid drift.
    """
    env = {}
...
def show_command():
    """Print the setup command, built from the <command> template in the
    docstring with {PLACEHOLDERS} filled from ../.env.

    Read-only: prints to stdout and does nothing else — no files, no sudo, no
    changes. Runs in the repo; the .env is resolved relative to this script
    (../.env), so the working directory does not matter.
    """
    # python3 -OO discards docstrings, leaving nothing to slice.
    if not MODULE_DOC:
        sys.exit("[show-command] Docstring unavailable — do not use "
                 "--show-command under python3 -OO.")

    # Slice out the block between the <command> tags. partition (not split)
    # stays regex-free and unambiguous.
    _, begin, rest = MODULE_DOC.partition("<command>")
    template, end, _ = rest.partition("</command>")
    if not begin or not end:
        sys.exit("[show-command] <command> template not found in the docstring.")
    # Drop the tag lines, then trim surrounding blank lines.
    ...
    try:
        ...
        sys.exit(f"[show-command] No .env at {env_path}; run inside the repo.")

    try:
        # Only the template is formatted; literals pass through, {PLACEHOLDERS}
        # are filled, and a missing key fails loudly.
        rendered = template.format(**env)
    except KeyError as missing:
        sys.exit(f"[show-command] {env_path} is missing key {missing}.")

    print(rendered)   # stdout only, clean for piping


def main():
    args = parse_args()
    # Read-only mode: print the command and exit before anything needs root.
    ...
    #     unless they should also be gated by the same service token.
    ...
    if args.cf_service_token_id and not args.cf_service_token_id.endswith('.access'):
        log.error(
            ...
            f"Make sure the Client ID was copied, not the token name or UUID."
        )
        sys.exit(1)
