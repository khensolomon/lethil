...
The bootstrap is fully scripted. `setup.py` provisions the VM, creates the Cloudflare Tunnel, ingress rules, DNS records, the Access application protecting SSH, and attaches a Service Auth policy that lets the GitHub Actions deploy pipeline authenticate. Credentials are supplied as flags; the script wires them up.
...
These are gathered once, stored in a password manager, and reused for every `setup.py` run. They are **never** committed to a repo or pushed to GitHub.
...
| Endpoint | Constructed from the account ID — no flag needed |

Both the Access Key ID and Secret Access Key are shown only once. Store them in the password manager alongside the other bootstrap credentials.

The endpoint URL is `https://<ACCOUNT_ID>.r2.cloudflarestorage.com`. `setup.py` builds it from `--cf-account-id` automatically; pass `--r2-account-id` only to use a different account for R2 than for the tunnel (rare).

### Store these safely

Suggested password-manager entry:
...
`server/setup.py` handles most of the provisioning: installs Docker, configures UFW, creates application directories, brings up the Cloudflare Tunnel and the landing-page nginx, creates DNS records, and configures the Access application that gates SSH.

There are two ways to run it: from GitHub Actions (preferred — secrets come from `.env` synced to GitHub Secrets, no hand-typing), or by hand on the server (the traditional way; same script, same flags).

### Option A — Dispatch from GitHub Actions (preferred)

1. Confirm `.env` at the repo root has the required values (see `.env.example`).
2. Sync them to GitHub Secrets: `python3 script/secrets.py --push`.
3. In the GitHub UI: **Actions → server-setup → Run workflow**. Fill in the form (target host, optional `force`/`dry-run`).
4. The workflow ships `server/setup.py` to the target via SSH and runs it with all the secrets as flags. Output appears in the workflow log.

This route suits provisioning a new server: every credential comes from the same source of truth (`.env`), with nothing to mistype.

### Option B — Run by hand on the server

Transfer the script and run it directly. Useful for one-off debugging or when GitHub Actions isn't available.

```bash
scp -i ~/.ssh/prod_server server/setup.py root@<server-ip>:/root/setup.py
```

### Robotic mode — recommended

Run with all five required credentials. `--app-domain` is repeatable; supply one per public hostname the apps need. `--r2-access-key-id` and `--r2-secret-access-key` are optional — supply them to have `setup.py` write a pre-configured rclone remote for R2.
...
admin_subdomains = {
    ...
    "terminal": {
        ...
        "ca_cert": True,   # app holding the short-lived certificate CA
    },
...
The `terminal` entry is different. It uses `ssh://` deliberately (the browser-render flow expects it) and omits `protect_with_access_app`, so `setup.py` creates only its DNS record and ingress rule — no Access app, policy, or service-token wiring. The rest of the browser-terminal setup (the self-hosted app and its email policy) is configured once in the dashboard. The short-lived-certificate CA it relies on, however, can be trusted on the server automatically: pass `--add-ca-public-key` (see below). The CA itself is still generated by hand in the dashboard — `setup.py` only resolves the Access app flagged `ca_cert` in `admin_subdomains` (the terminal app), fetches that app's CA, and trusts an existing one. It never creates the CA.
...
| Generate CA | Zero Trust → Access controls → Service credentials → SSH | Select Add a certificate, choose the terminal app in the Application dropdown, Generate certificate. A row named after the app appears (Certificate type: Application). One-time, by hand |
| Trust the CA | The server | Automated: run `setup.py --add-ca-public-key` (robotic mode), which resolves the `ca_cert`-flagged app, fetches its CA, and wires `/etc/ssh/ca.pub` + `sshd_config`. Or pass the key directly with `--ca-public-key "ecdsa-sha2-nistp256 AAAA... open-ssh-ca@cloudflareaccess.org"` |
...
| Docker CE | Installs Docker Engine + Compose plugin + Buildx; configures log rotation; adds the invoking user to the `docker` group so `docker` works without `sudo` (run `newgrp docker` after install to activate it in the current shell without re-login) |
...
With `setup.py` complete, the Cloudflare tunnel is up, ingress rules are routing, and a placeholder landing page is serving on port 80. Two items remain before the server is done.
...
```bash
cloudflared access tcp --hostname ssh.<admin-domain> --url localhost:2222 &
```
...
> **Two `secrets.py` scripts exist.** This section is about the **per-app** one — it lives in each Django app repo (e.g. myordbok, zaideih), reads that app's own `.env`, and pushes secrets to that app's GitHub repo so its deploy pipeline (`deploy.yml`) can authenticate. There is also `lethil/script/secrets.py` which does the same kind of thing but for the `lethil` infra repo's own `.env` — driving the `server-setup` and `apps-deploy` workflows. Different `.env` files, different target repos, same idea.
...
`CF_SERVICE_TOKEN_ID` / `CF_SERVICE_TOKEN_SECRET` are the same values passed to `setup.py`. One credential, three locations: password manager (durable), `.env` (per-app local working copy), GitHub Secret (consumed by the runner).
...
| `SERVER_HOSTNAME` | `.env` Zone 3 — usually `ssh.<admin-domain>` | `deploy_via_tunnel`, `deploy_via_ssh` |
...
> **The service token is durable.** Its credentials are inputs to `setup.py`, stored in the password manager and reused across runs. There is no secret on the VM that gets lost on reinstall. Tearing down the VM and provisioning a fresh one with the same flags reattaches the same service token to a fresh Access policy on a fresh tunnel — the apps' GitHub secrets continue to work without rotation.

### Rotating the service token (when required)

Service tokens expire, and proactive rotation after team changes is sometimes warranted. The flow:
...
2. Update the password manager with the new ID and secret
...
Order matters: keep the old token until step 5 succeeds, or in-flight deploys will fail.
...
**2.** The Access app has a **Service Auth** policy (not "Allow") referencing the service token. Click the app → Policies tab. The policy's **Action** must be **Service Auth**; **Action: Allow** with a service token in the include rule will produce this exact error.
...
If the GitHub Actions run shows `cp: '/usr/bin/cloudflared' and '/usr/local/bin/cloudflared' are the same file`, the runner image already has `cloudflared` preinstalled as a symlink. Use `dpkg -i --force-overwrite` and drop any manual `cp` step — `dpkg` will manage the binary location itself.
...
- `--cf-service-token-id` is wrong (typo, or the token name/UUID was copied instead of the Client ID — Client ID always ends in `.access`)
...
**2.** Confirm the endpoint matches the account ID. The line should read `endpoint = https://<account-id>.r2.cloudflarestorage.com`. Account ID is the same one in the Cloudflare dashboard sidebar, not the zone ID.

**3.** Confirm the R2 API token has access to the bucket being listed. R2 tokens can be scoped per-bucket — `Apply to all buckets` is permissive but auditable; per-bucket scoping is safer but easier to misconfigure.
...
`setup.py` adds the invoking user to the `docker` group, but Linux only refreshes group memberships when a process starts. The current shell started before `setup.py` ran, so it still has the old groups.
...
If `docker` appears in the output, the membership is active. If `setup.py` ran but did not add the account to the group (for example, run directly as root with no `SUDO_USER` set), add it manually:
...
The watch-out: shell scripts and command substitutions like `$(docker ps -q -f name=...)` run in *subshells*, which inherit the parent shell's groups. So `sudo docker exec $(docker ps -q ...)` runs the outer command as root but the inner one as the normal user — and the inner one fails with the permission error above, leaving the outer `docker exec` with no container ID to operate on. Fix the group membership instead of sprinkling `sudo` inside command substitutions.

### Landing page doesn't show, or assets return 502

The landing page should appear when the server is reached over plain HTTP, with or without a hostname header:
...
Both should return `200 OK` with `Server: nginx/...` and a `Content-Length` matching the `/opt/apps/html/index.html` file size.
...
If `nginx-proxy-manager` appears in the output, follow the upgrade-cleanup note in Section 5.
