/* browse.js — the interactive "all pages" index. Fetches /browse.json once,
...
(function () {
  ...
  var grid = document.querySelector("[data-browse-grid]");
  if (!grid) return;

  var catBox = document.querySelector("[data-browse-categories]");
  var tagBox = document.querySelector("[data-browse-tags]");
  var countEl = document.querySelector("[data-browse-count]");
  var emptyEl = document.querySelector("[data-browse-empty]");
  var clearBtn = document.querySelector("[data-browse-clear]");
  ...
  var pagerEl = document.querySelector("[data-browse-pager]");

  // deep-link support: /browse/#tag=docker&category=Server (from the /tags/ and
  ...
  function chip(label, count, kind) {
    ...
    b.type = "button";
    b.className = "browse__chip";
    b.dataset.kind = kind;
    b.dataset.value = label;
    b.setAttribute("aria-pressed", "false");
    b.innerHTML = "<span class=\"browse__chip-label\"></span>" +
      "<span class=\"browse__chip-count mono\"></span>";
    b.querySelector(".browse__chip-label").textContent = label;
    var countEl2 = b.querySelector(".browse__chip-count");
  ...
  function setupCollapse() {
    ...
    Array.prototype.forEach.call(groups, function (group) {
      ...
      function refresh() {
        ...
        if (overflowing) {
          var count = box.querySelectorAll(".browse__chip").length;
  ...
  function onChip(ev) {
    var b = ev.target.closest(".browse__chip");
    if (!b) return;
  ...
  function card(d) {
    ...
    a.className = "browse__card";
    a.href = d.url;
    a.innerHTML =
      "<span class=\"browse__card-section mono\"></span>" +
      "<span class=\"browse__card-title\"></span>" +
      "<span class=\"browse__card-desc\"></span>" +
      "<span class=\"browse__card-tags\"></span>";
    a.querySelector(".browse__card-section").textContent = d.section;
    a.querySelector(".browse__card-title").textContent = d.title;
    a.querySelector(".browse__card-desc").textContent = d.description || "";
    // tags on the card are themselves filter buttons — clicking one adds it to
    // the tag filter instead of navigating to the page.
    var tagWrap = a.querySelector(".browse__card-tags");
    (d.tags || []).slice(0, 5).forEach(function (t) {
      var tb = document.createElement("button");
      tb.type = "button";
      tb.className = "browse__card-tag mono";
      tb.textContent = t;
      tb.dataset.tag = t;
  ...
  function goToPage(n, total) {
    ...
    var top = document.querySelector(".browse__controls");
  ...
  function pageButton(label, targetPage, totalPages, opts) {
    ...
    var b = document.createElement("button");
    b.type = "button";
    b.className = "browse__page" + (opts.current ? " is-current" : "");
    b.textContent = label;
  ...
  function renderPager(totalItems) {
    ...
    nums.forEach(function (n) {
      if (n === "…") {
        ...
        s.className = "browse__page-gap"; s.textContent = "…";
