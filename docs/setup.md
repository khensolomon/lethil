...
The `terminal` entry is different. It uses `ssh://` deliberately (the browser-render flow expects it) and omits `protect_with_access_app`, so `setup.py` creates only its DNS record and ingress rule — no Access app, policy, or service-token wiring. The rest of the browser-terminal setup (the self-hosted app and its email policy) is configured once in the dashboard. The short-lived-certificate CA it relies on, however, can be trusted on the server automatically: pass `--add-ca-public-key` (see below). The CA itself is still generated by hand in the dashboard — `setup.py` only fetches the account SSH CA (`gateway_ca`) and trusts an existing one, never creates it.
...
| Generate CA | Zero Trust → Access controls → Service credentials → SSH | Generate the SSH CA (under "SSH with Access for Infrastructure", select Generate SSH CA). Account-wide, one-time, by hand |
| Trust the CA | The server | Automated: run `setup.py --add-ca-public-key` (robotic mode), which fetches the account SSH CA via `gateway_ca` and wires `/etc/ssh/ca.pub` + `sshd_config`. Or pass the key directly with `--ca-public-key "ssh-... AAAA"` |
