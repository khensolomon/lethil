...
| Generate CA | Zero Trust → Access controls → Service credentials → SSH | Generate the short-lived-certificate CA (Resource: terminal, Certificate type: Application) for the app. One-time, by hand |
| Trust the CA | The server | Automated: run `setup.py --add-ca-public-key` (robotic mode), which fetches the app CA and wires `/etc/ssh/ca.pub` + `sshd_config`. Or pass the key directly with `--ca-public-key "ssh-... AAAA"` |
