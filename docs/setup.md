...
`server/setup.py` does almost everything: installs Docker, configures UFW, creates application directories, brings up the Cloudflare Tunnel and the landing-page nginx, creates DNS records, and configures the Access application that gates SSH.
...
1. Make sure `.env` at the repo root has the required values (see `.env.example`).
...
This is the route to use when provisioning a new server, because every credential comes from the same source-of-truth (`.env`) and there's nothing to typo.
